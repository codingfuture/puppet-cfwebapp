...
Alerta Server API is installed from GitHub (by default) with FutoIn CID and run through uWSGI.
...
* `futoin` app shortcuts:
    ...
    * `$deploy_type = 'vcstag'`
    * `$deploy_tool = 'git'`
    * `$deploy_url = 'https://github.com/alerta/alerta.git'`
    * `$deploy_match = 'v5*'`
* Alerta-specific:
...
Alerta Web UI is installed from GitHub (by default) with FutoIn CID and served via nginx
with no backend processing.
...
* `futoin` app shortcuts:
    ...
    * `$deploy_match = 'v5*'`
* Alerta-specific:
    * `$api_endpoint = 'http://localhost.localdomain'` - Alerta Server API
    * `$tune = {}` - fine overrides
